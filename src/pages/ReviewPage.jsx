...
export default function ReviewPage({ session, onDone, onBack }) {
  ...
  // Two-layer focus grab on mount so keyboard works immediately in Tauri 2 / WebView2:
  // OS layer → getCurrentWindow().setFocus()
  // DOM layer → rootRef.current.focus()
  useEffect(() => {
  ...
  // Keyboard handler (fallback path; on Windows the hook swallows arrows/space
  // before they reach here, so this fires only on non-Windows dev builds)
  useEffect(() => {
  ...
  // Rust hook → mark://nav → move MARK's video (programmatic DOM, no user-gesture restriction)
  useEffect(() => {
    let unlisten
    listen('mark://nav', (e) => {
  ...
  // Rust worker → mark://sync-status → update connection dot
  useEffect(() => {
    let unlisten
  ...
  function togglePlay() {
    ...
    if (v.paused) {
      v.play()
        ...
        .catch(err => console.error('[MARK] play() failed:', err))
    } else {
      v.pause()
      setPlaying(false)
    }
